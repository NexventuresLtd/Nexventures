...
export default function Navbar({ className = '' }: NavbarProps) {
  ...
  return (
    <motion.header
      ref={navbarRef}
      className={`sticky top-0 z-50 transition-all duration-300 backdrop-blur-lg ${isScrolled
        ? darkMode
    ...
    >
      ...
      <div className="max-w-full md:max-w-11/12 mx-auto px-4 py-4">
        <div className="flex justify-between items-center">
            ...
            <div className="flex items-center">
              ...
              >
                <div>
                  ...
                  <div className={`text-xs font-normal -mt-1 ${!darkMode ? 'text-gray-400' : 'text-gray-500'
                    }`}>
            ...
            {navItems.map((item) => (
              <div key={item.to} className="relative">
                ...
                >
                  <Link
                    to={item.to}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#952301] group ${isActive(item.to)
                  ...
                  >
                    ...
                    {item.hasDropdown && (
                      <ChevronDown
                        size={14}
                        className={`transition-transform ${activeDropdown === item.to ? 'rotate-180' : ''
                          }`}
                      />
                    )}
                  </Link>
                  ...
                  <AnimatePresence>
                    {item.hasDropdown && activeDropdown === item.to && (
                      <motion.div
                        className={`absolute top-full left-0 mt-2 w-64 rounded-xl border overflow-hidden z-50 ${darkMode
                        ...
                        exit="exit"
                      >
                        <div className={`p-3 bg-gradient-to-r from-[#952301]/5 to-[#611701]/5 border-b ${!darkMode ? 'border-gray-700' : 'border-gray-100'
                          }`}>
          ...
          <div className="hidden lg:flex items-center space-x-3">
            ...
            <motion.button
              onClick={toggleTheme}
              className={`p-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#952301] ${darkMode
          ...
          {/* Mobile Menu Button */}
          <motion.button
            className={`lg:hidden p-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-[#952301] ${darkMode
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.nav
            className={`lg:hidden border-t overflow-hidden ${darkMode
          ...
          >
            <div className="px-4 py-6 space-y-4">
              ...
              <form onSubmit={handleSearch} className="relative mb-6">
                <Search size={18} className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${!darkMode ? 'text-gray-400' : 'text-gray-400'
                  }`} />
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  placeholder="Search..."
                  className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#952301] transition-colors ${darkMode
              ...
              <div className="space-y-2">
                {navItems.map((item, index) => (
                  ...
                  >
                    <Link
                      to={item.to}
                      className={`flex items-center gap-3 p-3 rounded-lg font-medium transition-colors ${isActive(item.to)
                        ? 'text-[#952301] bg-[#952301]/10'
                        : darkMode
                          ? 'text-gray-300 hover:text-[#952301] hover:bg-[#952301]/10'
                          : 'text-gray-700 hover:text-[#952301] hover:bg-[#952301]/5'
              ...
              {/* Mobile Actions */}
              <div className={`pt-6 border-t space-y-4 ${!darkMode ? 'border-gray-700' : 'border-gray-200'
                }`}>
                <button
                  onClick={toggleTheme}
                  className={`flex items-center gap-3 w-full p-3 transition-colors rounded-lg ${darkMode
                    ? 'text-gray-300 hover:text-yellow-400 hover:bg-gray-800'
